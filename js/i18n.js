 ...
 *   data-i18n-html="key"        → replaces element.innerHTML (use sparingly)
 *   data-i18n-placeholder="key" → replaces input placeholder
 */
...
(function () {
  ...
  const T = {
    ...
    /* ── ENGLISH (default — keys match the HTML text exactly) ── */
    en: {},   // empty: applyLang('en') is a no-op restore pass
    ...
    de: {
      ...
      'card5.why':          'Der körperliche Übergang in den Schlaf ist ein zweistündiger biologischer Prozess, den die meisten Menschen mit Bildschirmen überschreiben. Alles in dieser Kategorie unterstützt diesen Übergang.',
      ...
      'access.emailLabel':  'Geben Sie Ihre E-Mail-Unterschrift ein',
    ...
    pt: {
      ...
      'card1.why':          'Os primeiros 90 minutos do dia definem o ritmo das próximas 12 horas. Tudo nesta categoria é selecionado para manter suas mãos ocupadas, seus olhos longe das telas e seu sistema nervoso em seu ritmo natural de despertar.',
  ...
  function cacheOriginals() {
    ...
    // system h2 stored separately
    const h2 = document.getElementById('system-h2');
    if (h2) originals['system-h2__html'] = h2.innerHTML;
    // step2 CTA
    const cta = document.getElementById('step2-cta');
    if (cta) originals['step2-cta__html'] = cta.innerHTML;
    // hero CTA first text node
    ...
    // email label (for when browser stores placeholder separately)
  }
  ...
  function applyLang(lang) {
    ...
    if (!t || lang === 'en') {
      ...
      // System h2 (contains a <br> and a <span>)
      const h2 = document.getElementById('system-h2');
      ...
      // Hero CTA (preserve the ↓ span child)
      const heroCta = document.querySelector('.hero-cta');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // Restore saved preference before animations fire (body.loaded fires ~1500ms later)
    let saved = 'en';
  ...
  // Expose globally so external callers can trigger a language change if needed
